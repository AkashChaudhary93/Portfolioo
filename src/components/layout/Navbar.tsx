  ...
  return (
    <nav className="sticky top-0 z-50 bg-cream border-b-4 border-black">
      <div className="w-full max-w-[1600px] mx-auto px-6 md:px-12 py-4 flex flex-wrap items-center justify-between gap-4 lg:gap-6">
        <Link
          href="/"
          className="inline-flex items-center bg-white border-4 border-black px-4 py-2 shadow-[5px_5px_0px_0px_rgba(0,0,0,1)] arcade-hover arcade-glow"
        >
          <span className="font-black text-2xl md:text-3xl tracking-tight uppercase text-black coin-spin">DEV</span>
          <span className="font-black text-2xl md:text-3xl tracking-tight uppercase text-hot-orange ml-1 coin-spin">PORTFOLIO</span>
        </Link>
        ...
        <div className="hidden lg:flex items-center bg-white border-4 border-black px-5 py-3 shadow-[5px_5px_0px_0px_rgba(0,0,0,1)] gap-5 xl:gap-7 flex-wrap">
          ...
          <Link
            href="#contact"
            className="inline-flex items-center justify-center bg-lime-green border-4 border-black px-4 py-2 font-black uppercase tracking-widest text-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] arcade-hover transition-all"
          >
            HIRE ME
          </Link>
        </div>
        ...
        <div className="lg:hidden w-full flex items-center justify-between gap-3 bg-white border-4 border-black px-4 py-3 shadow-[5px_5px_0px_0px_rgba(0,0,0,1)]">
          ...
          <Link
            href="#contact"
            className="inline-flex items-center justify-center bg-lime-green border-4 border-black px-4 py-2 font-black uppercase text-sm tracking-widest text-black arcade-hover shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"
          >
            HIRE ME
          </Link>
        </div>
...
const NavLink = ({ href, text }: { href: string; text: string }) => (
  <Link href={href} className="font-black uppercase tracking-widest text-black hover:text-hot-orange transition-colors text-sm xl:text-base arcade-hover">
    {text}
  </Link>
);

const CoinHud = ({ compact = false }: { compact?: boolean }) => (
  <div className={`inline-flex items-center gap-2 border-4 border-black bg-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] pac-waddle ${compact ? 'px-2 py-1.5' : 'px-3 py-2'}`}>
...
const StatusChip = ({ compact = false }: { compact?: boolean }) => (
  <div className={`inline-flex items-center gap-2 border-4 border-black bg-white px-3 py-2 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] pac-waddle ${compact ? 'hidden sm:inline-flex' : ''}`}>
...
const HeartBar = ({ compact = false }: { compact?: boolean }) => {
  const hearts = Array.from({ length: 5 }).map((_, i) => (
    <span
      key={i}
      className={`inline-flex items-center justify-center border-2 border-black leading-none shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] pac-waddle ${
...
const LevelBadge = ({ compact = false }: { compact?: boolean }) => (
  <div className={`inline-flex items-center border-4 border-black bg-sky-blue shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] pac-waddle ${compact ? 'px-2 py-1' : 'px-3 py-2'}`}>
